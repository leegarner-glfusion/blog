...
// +--------------------------------------------------------------------------+
// | $Id:: services.inc.php 5952 2010-04-17 03:14:27Z mevans0263             $|
// +--------------------------------------------------------------------------+
// |                                                                          |
...
function service_submit_article($args, &$output, &$svc_msg)
{
    ...
    if (empty($args['group_id'])) {
        $args['group_id'] = SEC_getFeatureGroup('article.edit', $_USER['uid']);
    }
    ...
    if ($gl_edit && !empty($args['gl_etag'])) {
        ...
        $o = array();
        $s = array();
        $r = service_get_article(array('sp_id' => $sp_old_id, 'gl_svc' => true), $o, $s);
    ...
    if ($duplicate_id) {
        ...
        if ($_SP_CONF['filter_html'] == 1) {
            $sp_content = COM_checkHTML ($sp_content,'article.edit');
        }
        ...
        if (($_SP_CONF['allow_php'] != 1) || !SEC_hasRights ('article.PHP')) {
            $sp_php = 0;
        }
        ...
        DB_save($_TABLES['staticpage'], 
                'sp_id,sp_status,sp_uid,sp_title,sp_content,sp_date,sp_hits,sp_format,sp_onmenu,sp_label,commentcode,owner_id,group_id,'
        ...
        if ($delete_old_page && !empty ($sp_old_id)) {
            ...
            DB_change($_TABLES['comments'], 'sid', DB_escapeString($sp_id),
                      array('sid', 'type'),
                      array(DB_escapeString($sp_old_id), 'article'));
            PLG_itemDeleted($sp_old_id, 'article');

        }
        PLG_itemSaved($sp_id,'article');
        $url = COM_buildURL($_CONF['site_url'] . '/article/index.php?page='
                            . $sp_id);
 ...
 * @return  int		    Response code as defined in lib-plugins.php
 */
function service_delete_article($args, &$output, &$svc_msg)
{
    global $_CONF, $_TABLES, $_USER, $LANG_ACCESS, $LANG12, $LANG_STATIC,
           $LANG_LOGIN;

    $output = COM_refresh($_CONF['site_admin_url'] . '/plugins/article/index.php');
    ...
    if (!SEC_hasRights ('article.delete')) {
        $output = COM_siteHeader ('menu', $LANG_STATIC['access_denied']);
        $output .= COM_startBlock ($LANG_STATIC['access_denied'], '',
                                    COM_getBlockTemplate ('_msg_block', 'header'));
        $output .= $LANG_STATIC['access_denied_msg'];
    ...
    DB_delete($_TABLES['comments'], array('sid',  'type'),array($sp_id, 'article'));
    PLG_itemDeleted($sp_id, 'article');
 ...
 * @return  int		    Response code as defined in lib-plugins.php
 */
function service_get_article($args, &$output, &$svc_msg)
{
 ...
 * @return  int         Response code as defined in lib-plugins.php
 */
function service_getTopicList_article($args, &$output, &$svc_msg)
{
